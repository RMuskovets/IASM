...
def getOperands():
	OPER = 'mov add sub mul div and xor or not shl shr push pop int sets gets set get cmp jmp'.split()
	a = eval("r'(" + '|'.join(OPER) + ")'")
	return a

def getRegisters():
	REG = 'eax ebx ecx edx al ah bl bh cl ch dl dh ax bx cx dx eip esp ebp ip sp bp esi edi si di of df if tf sf zf af pf cf cs ds ss es fs gs'.split()
	a = eval("r'(" + '|'.join(REG) + ")'")
	return a
...
class Lexer(object):
	...
	def __init__(self,rules=RULES_DEFAULT, case_sensitive=True, omit_whitespace=True):
...
def test():
	lxr = Lexer()
	for x in lxr.scan('mov eax, ebx'):
		print (x)
	for x in lxr.scan("mov eax, 13"):
		print (x)